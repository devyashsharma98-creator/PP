...
import { createConferenceSchema, listConferencesQuerySchema } from "@/lib/validators/conferences";
...
export const GET = withAuth(async (req: NextRequest, ctx) => {
  ...
  const query = listConferencesQuerySchema.safeParse(Object.fromEntries(sp));
  ...
  const result = await conferenceService.listConferences(query.data, ctx.session.orgId, page, limit, offset);
  if (!result.ok) return result.response;
  return apiSuccess(result.data.rows, { meta: paginationMeta(page, limit, result.data.total) });
});

export const POST = withPermission("canCreateConference", async (req: NextRequest, ctx) => {
  const ip = getClientIp(req); const rateRes = withApiRateLimit(ip); if (rateRes) return rateRes;
  ...
  const result = await conferenceService.createConference(parsed.data, ctx);
  if (!result.ok) return result.response;
