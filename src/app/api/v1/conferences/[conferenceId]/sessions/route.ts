...
import { createConferenceSessionSchema } from "@/lib/validators/conferences";
...
export const GET = withAuth(async (req: NextRequest, ctx, params) => {
  ...
  const result = await conferenceService.listSessions(p.conferenceId);
  if (!result.ok) return result.response;
  return apiSuccess(result.data.rows);
});

export const POST = withPermission("canManageConferenceSessions", async (req: NextRequest, ctx, params) => {
  const ip = getClientIp(req); const rateRes = withApiRateLimit(ip); if (rateRes) return rateRes;
  const p = params as { conferenceId: string };
  if (!p?.conferenceId) return badRequest("Conference ID is required.");
  let body; try { body = await req.json(); } catch { return badRequest("Request body must be valid JSON."); }
  const parsed = createConferenceSessionSchema.safeParse(body);
  if (!parsed.success) return badRequest(parsed.error.errors[0]?.message ?? "Invalid input.");
  const result = await conferenceService.createSession(p.conferenceId, parsed.data, ctx);
  if (!result.ok) return result.response;
