...
import { createConferenceRegistrationSchema } from "@/lib/validators/conferences";
import * as conferenceService from "@/lib/server/services/conference-service";

export const GET = withPermission("canViewConferenceRegistrations", async (req: NextRequest, ctx, params) => {
  ...
  const result = await conferenceService.listRegistrations(p.conferenceId);
...
export const POST = withAuth(async (req: NextRequest, ctx, params) => {
  ...
  const parsed = createConferenceRegistrationSchema.safeParse(body);
  if (!parsed.success) return badRequest(parsed.error.errors[0]?.message ?? "Invalid input.");
  const result = await conferenceService.createRegistration(p.conferenceId, parsed.data, ctx);
  if (!result.ok) return result.response;
